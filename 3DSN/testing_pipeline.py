...
parser.add_argument('--p',type=str,nargs='+',help="file names")
parser.add_argument('--k',type=str,nargs='+',help="indices")
parser.add_argument('--r',type=str,help="voxel radius",default="0.15")
parser.add_argument('--h',type=str,help="kernel width",default="1.75")
...
point_cloud_files = args.p
keypoints_files = args.k
print(point_cloud_files)
print(keypoints_files)

for i in range(0,len(point_cloud_files)):
    sdv_args = "./3DSmoothNet -f " + point_cloud_files[i] + " -k " + keypoints_files[i] +  " -o ./data/train/sdv/"
    subprocess.call(sdv_args, shell=True)
    
    print('Input parametrization complete. Start inference')


# Run the inference as shell 
args = "python main_cnn.py --run_mode=test --evaluate_input_folder=./data/train/sdv/  --evaluate_output_folder=./data/train"
...
source_desc_name = "./data/train/32_dim/" + point_cloud_files[0] + "_0.150000_16_1.750000_3DSmoothNet.npz"
reference_desc = np.load(source_desc_name)
reference_desc = reference_desc['data']

dest_desc_name = "./data/train/32_dim/" + point_cloud_files[1] + "_0.150000_16_1.750000_3DSmoothNet.npz"
test_desc = np.load(dest_desc_name)
